...
from utils import BGR2YCrCb, YCrCb2RGB, correct_output_image, draw, load_model, postprocess_output, preprocess_image
...
class DFD:
    ...
    def preprocess(self):
        ...
        while cap_ir.isOpened() and cap_tm.isOpened():
            ...
            if ir_time >= ir_next_time and tm_time >= tm_next_time:
                ...
                tm_img = np.expand_dims(np.expand_dims(tm_img, axis=0), axis=0) # (1, 1, H, W) BGR
                ...
                self.preprocess_queue.put((idx, (enhanced_img, avg_brightness, tm_img))) # (H, W, 3) RGB, (1, 1, H, W) BGR
                idx += 1
    ...
    def dehaze(self):
        ...
        while True:
            ...
            enhanced_img, avg_brightness, tm_img = item # (H, W, 3) RGB, (1, 1, H, W) BGR
            ...
            ir_img = dehaze_img.astype(np.float32) / 255.0 # (H, W, 3) RGB
            tm_img = tm_img.astype(np.float32) / 255.0 # (1, 1, H, W) BGR

            ir_img = cv2.cvtColor(ir_img, cv2.COLOR_RGB2BGR) # (H, W, 3) BGR
            Y, Cr, Cb = BGR2YCrCb(ir_img) # (1, 1, H, W), (H, W), (H, W) BGR
            
            inputs = [Y, tm_img] # [(1, 1, H, W), (1, 1, H, W)] [BGR, BGR]
            self.dehaze_queue.put((idx, (inputs, Cr, Cb))) # [(1, 1, H, W), (1, 1, H, W)] [BGR, BGR]
            self.preprocess_queue.task_done()
