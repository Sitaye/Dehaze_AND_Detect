...
from utils import register_init, BGR2YCrCb, YCrCb2RGB, correct_output_image, draw, img_register, load_model, postprocess_output, preprocess_image
...
class DRFD:
    def __init__(self, config, stop_event=Event()):
        ...
        self.fuse_thread_count = config["thread"]["fuse"]["max_workers"]
        self.dehaze_thread_count = config["thread"]["dehaze"]["max_workers"]
    ...
    def preprocess(self):
        ...
        while cap_ir.isOpened() and cap_tm.isOpened():
            ...
            if ir_time >= ir_next_time and tm_time >= tm_next_time:
                ...
                tm_img = cv2.resize(tm_img, (self.W, self.H), interpolation=cv2.INTER_LANCZOS4) # (H, W) BGR
                ...
                self.preprocess_queue.put((idx, (enhanced_img, avg_brightness, tm_img))) # (H, W, 3) RGB, (H, W) BGR
                idx += 1
    ...
    def dehaze(self):
        ...
        while True:
            ...
            if item is self.SIGNAL:
                # 得到数据传输完毕的通知，关闭线程，并通知配准线程有一个去烟线程已经处理完毕
                self.dehaze_queue.put((self.FINAL, self.SIGNAL))
                self.preprocess_queue.task_done()
                break

            enhanced_img, avg_brightness, tm_img = item # (H, W, 3) RGB, (H, W) BGR
            ...
            if avg_brightness > 150:
                ...
                dehaze_img = postprocess_output(outputs[0]) # (H, W, 3) RGB

            ir_img = cv2.cvtColor(dehaze_img, cv2.COLOR_RGB2BGR) # (H, W, 3) BGR
            self.dehaze_queue.put((idx, (ir_img, tm_img))) # (H, W, 3) BGR (H, W) BGR
            self.preprocess_queue.task_done()
    ...
    def register(self):
        registration_method = register_init()
        count = 0
        while True:
            # 外部中断
            if self.stop_event.is_set():
            ...
            idx, item = self.dehaze_queue.get()
            if item is self.SIGNAL:
                ...
                if self.dehaze_finished_count == self.dehaze_thread_count:
                    ...
                    for _ in range(self.fuse_thread_count):
                        self.register_queue.put((self.FINAL, self.SIGNAL))
                    break
                continue

            ir_img, tm_img = item # [(H, W, 3), (H, W)]
            
            # 判断是否为关键帧，假如关键帧为 每 5 帧
            if count % 5 == 0:
                is_key = True
            else:
                is_key = False
            ...
            ir_img, tm_img, self.trasnform = img_register(
                ir_img=ir_img,
                tm_img=tm_img,
                registration_method=registration_method,
                transform=self.trasnform,
                is_key=is_key,
            )

            Y, Cr, Cb = BGR2YCrCb(ir_img) # (1, 1, H, W), (H, W), (H, W) BGR
            inputs = [Y, tm_img] # [(1, 1, H, W), (1, 1, H, W)] [BGR, BGR]
            self.register_queue.put((idx, (inputs, Cr, Cb))) # [(1, 1, H, W), (1, 1, H, W)] [BGR, BGR]
            self.dehaze_queue.task_done()
    ...
    def fuse(self):
        ...
        while True:
            ...
            idx, item = self.register_queue.get()
            if item is self.SIGNAL:
                # 得到数据传输完毕的通知，关闭线程，并通知融合线程有一个去烟线程已经处理完毕
                self.fuse_queue.put((self.FINAL, self.SIGNAL))
                self.register_queue.task_done()
                break
    ...
    def reset(self):
        ...
        self.dehaze_queue = Queue(maxsize=self.maxsize)
        self.register_queue = Queue(maxsize=self.maxsize)
    ...
    def release(self):
        ...
        with self.register_queue.mutex:
            self.register_queue.queue.clear()
